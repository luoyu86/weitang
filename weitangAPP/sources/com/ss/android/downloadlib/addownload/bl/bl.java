...
public class bl {
        ...
        for (int i2 = 0; i2 < successedDownloadInfosWithMimeType.size(); i2++) {
            ...
            if (downloadInfo != null) {
                ...
                if (file.exists()) {
                    ...
                    long jOk = ((long) com.ss.android.socialbase.downloader.h.ok.ok(downloadInfo.getId()).ok("download_complete_file_expire_hours", 0)) * 3600000;
                    if (jOk <= 0) {
                        jOk = 604800000;
                    }
                    boolean z = true;
                    if (jCurrentTimeMillis < jOk && !j.kf(r.getContext(), str)) {
                        z = false;
                    }
                    if (z) {
                        ok(file);
    ...
    public static void ok() throws Throwable {
        List<DownloadInfo> listOk = com.ss.android.socialbase.appdownloader.s.k().ok(r.getContext());
        if (listOk == null || listOk.size() <= 0) {
            return;
        }
        for (int i2 = 0; i2 < listOk.size(); i2++) {
            DownloadInfo downloadInfo = listOk.get(i2);
            File file = new File(downloadInfo.getTempPath(), downloadInfo.getTempName());
            long jLastModified = file.lastModified();
            long jOk = ((long) com.ss.android.socialbase.downloader.h.ok.ok(downloadInfo.getId()).ok("download_file_expire_hours", 0)) * 3600000;
            if (jOk <= 0) {
                jOk = 604800000;
            }
            if (file.isFile() && file.exists() && System.currentTimeMillis() - jLastModified >= jOk) {
                ok(file);
    ...
    */
    public static void ok(java.io.File r3) throws java.lang.Throwable {
        /*
            r0 = 0
