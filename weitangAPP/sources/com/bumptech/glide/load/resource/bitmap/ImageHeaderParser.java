...
public class ImageHeaderParser {
    ...
    public static class StreamReader {
        ...
        public int read(byte[] bArr) throws IOException {
            ...
            while (length > 0) {
                int i2 = this.is.read(bArr, bArr.length - length, length);
                if (i2 == -1) {
                    break;
                }
                length -= i2;
    ...
    private byte[] getExifSegment() throws IOException {
        ...
        do {
            ...
            if (uInt8 == 225) {
                ...
                int i2 = this.streamReader.read(bArr);
                if (i2 == uInt16) {
                    return bArr;
                }
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "Unable to read segment data, type: " + ((int) uInt8) + ", length: " + uInt16 + ", actually read: " + i2);
                }
                return null;
            }
            j = uInt16;
