...
public class ListInfo {
    ...
    public ListInfo(IMAPResponse iMAPResponse) throws ParsingException {
        ...
        if (simpleList != null) {
            for (int i2 = 0; i2 < simpleList.length; i2++) {
                if (simpleList[i2].equalsIgnoreCase("\\Marked")) {
                    this.changeState = 1;
                } else if (simpleList[i2].equalsIgnoreCase("\\Unmarked")) {
                    this.changeState = 2;
                } else if (simpleList[i2].equalsIgnoreCase("\\Noselect")) {
                    this.canOpen = false;
                } else if (simpleList[i2].equalsIgnoreCase("\\Noinferiors")) {
                    this.hasInferiors = false;
                }
                arrayList.add(simpleList[i2]);
            }
        }
