...
public class IdleManager {
        ...
        while (true) {
            ...
            if (channel != null) {
                ...
                if (socket == null || socket.getSoTimeout() <= 0) {
                    ...
                    this.f10185es.execute(new Runnable() { // from class: com.sun.mail.imap.IdleManager.2
                        @Override // java.lang.Runnable
                        public void run() {
        ...
        while (!this.die) {
            try {
                try {
                    try {
                        try {
                            ...
                            this.logger.log(Level.FINEST, "IdleManager got exception", (Throwable) e2);
                            this.die = true;
                    ...
                    this.logger.log(Level.FINEST, "IdleManager interrupted", (Throwable) e6);
                    this.die = true;
                    this.logger.finest("IdleManager unwatchAll");
                    try {
                        unwatchAll();
                ...
                try {
                    ...
                    this.logger.log(Level.FINEST, "IdleManager unwatch exception", (Throwable) e8);
                }
        ...
        while (true) {
            ...
            if (channel != null) {
                try {
                    ...
                    this.logger.log(Level.FINEST, "IdleManager exception while aborting idle for folder: " + folderName(iMAPFolderPoll), (Throwable) e3);
                }
            }
        }
    }
    ...
    private void watchAll() {
        while (true) {
            ...
            try {
                ...
                this.logger.log(Level.FINEST, "IdleManager can't register folder", (Throwable) e3);
            }
        }
    }
    ...
    public void watch(Folder folder) throws MessagingException {
        ...
        if (mailLogger3.isLoggable(level3)) {
            if (i2 > 0) {
                this.logger.log(level3, "IdleManager.watch startIdle succeeded for {0} after " + i2 + " tries", folderName(iMAPFolder));
            } else {
