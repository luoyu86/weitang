...
public final class MultiFormatUPCEANReader extends OneDReader {
    ...
    public Result decodeRow(int i2, BitArray bitArray, Map<DecodeHintType, ?> map) throws NotFoundException {
        ...
        for (UPCEANReader uPCEANReader : this.readers) {
            try {
                Result resultDecodeRow = uPCEANReader.decodeRow(i2, bitArray, iArrFindStartGuardPattern, map);
                boolean z = resultDecodeRow.getBarcodeFormat() == BarcodeFormat.EAN_13 && resultDecodeRow.getText().charAt(0) == '0';
                Collection collection = map == null ? null : (Collection) map.get(DecodeHintType.POSSIBLE_FORMATS);
                return (z && (collection == null || collection.contains(BarcodeFormat.UPC_A))) ? new Result(resultDecodeRow.getText().substring(1), null, resultDecodeRow.getResultPoints(), BarcodeFormat.UPC_A) : resultDecodeRow;
