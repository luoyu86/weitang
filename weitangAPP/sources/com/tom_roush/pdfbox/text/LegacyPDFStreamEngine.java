...
public class LegacyPDFStreamEngine extends PDFStreamEngine {
    ...
    public float computeFontHeight(PDFont pDFont) throws IOException {
        ...
        if (fontDescriptor != null) {
            ...
            if (capHeight > ascent && ascent > 0.0f && descent < 0.0f) {
                float f2 = (ascent - descent) / 2.0f;
                if (f2 < height || Float.compare(height, 0.0f) == 0) {
                    height = f2;
                }
            }
        }
    ...
    public void showGlyph(Matrix matrix, PDFont pDFont, int i2, String str, Vector vector) throws IOException {
        ...
        Matrix matrixMultiply = Matrix.getTranslateInstance(x * fontSize * horizontalScaling, vector.getY() * fontSize).multiply(textMatrix).multiply(currentTransformationMatrix);
        float translateX = matrixMultiply.getTranslateX();
        float translateY = matrixMultiply.getTranslateY();
        float translateX2 = translateX - matrix.getTranslateX();
        Float fValueOf = this.fontHeightMap.get(pDFont.getCOSObject());
        if (fValueOf == null) {
            fValueOf = Float.valueOf(computeFontHeight(pDFont));
            this.fontHeightMap.put(pDFont.getCOSObject(), fValueOf);
        }
        float scalingFactorY = matrix.getScalingFactorY() * fValueOf.floatValue();
        float scaleX = pDFont instanceof PDType3Font ? pDFont.getFontMatrix().getScaleX() : 0.001f;
        try {
            averageFontWidth = pDFont.getSpaceWidth() * scaleX;
        } catch (Throwable th) {
            Log.w("PdfBox-Android", th.getMessage(), th);
            averageFontWidth = 0.0f;
        }
        if (averageFontWidth == 0.0f) {
            averageFontWidth = pDFont.getAverageFontWidth() * scaleX * 0.8f;
        }
        if (averageFontWidth == 0.0f) {
            averageFontWidth = 1.0f;
        }
        float scalingFactorX = averageFontWidth * matrix.getScalingFactorX();
        String unicode = pDFont.toUnicode(i2, GLYPHLIST);
        if (unicode != null) {
            str2 = unicode;
        ...
        if (matrix2 == null) {
            matrixConcatenate = matrix;
        } else {
            matrixConcatenate = Matrix.concatenate(matrix2, matrix);
            translateX -= this.pageSize.getLowerLeftX();
            translateY -= this.pageSize.getLowerLeftY();
        }
        processTextPosition(new TextPosition(this.pageRotation, this.pageSize.getWidth(), this.pageSize.getHeight(), matrixConcatenate, translateX, translateY, Math.abs(scalingFactorY), translateX2, Math.abs(scalingFactorX), str2, new int[]{i2}, pDFont, fontSize, (int) (textMatrix.getScalingFactorX() * fontSize)));
    }
}
