...
public class PDFObjectStreamParser extends BaseParser {
    ...
    public PDFObjectStreamParser(COSStream cOSStream, COSDocument cOSDocument) throws IOException {
        ...
        int i2 = cOSStream.getInt(COSName.N);
        this.numberOfObjects = i2;
        if (i2 == -1) {
            throw new IOException("/N entry missing in object stream");
        }
        if (i2 < 0) {
            throw new IOException("Illegal /N entry in object stream: " + i2);
        }
        int i3 = cOSStream.getInt(COSName.FIRST);
        this.firstObject = i3;
        if (i3 == -1) {
        ...
        throw new IOException("Illegal /First entry in object stream: " + i3);
    }

    private COSBase parseObject(int i2) throws IOException {
        long position = this.seqSource.getPosition();
        int i3 = this.firstObject + i2;
        if (i3 > 0 && position < i3) {
            this.seqSource.readFully(i3 - ((int) position));
        }
    ...
    private Map<Integer, Long> readOffsets() throws IOException {
        ...
        long position = (this.seqSource.getPosition() + ((long) this.firstObject)) - 1;
        for (int i2 = 0; i2 < this.numberOfObjects && this.seqSource.getPosition() < position; i2++) {
