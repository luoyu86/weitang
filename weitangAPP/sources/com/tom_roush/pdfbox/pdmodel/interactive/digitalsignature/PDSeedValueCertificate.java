...
public class PDSeedValueCertificate implements COSObjectable {
    ...
    private static List<byte[]> getListOfByteArraysFromCOSArray(COSArray cOSArray) {
        ...
        for (COSBase cOSBase : cOSArray) {
            if (cOSBase instanceof COSString) {
                linkedList.add(((COSString) cOSBase).getBytes());
            }
        }
        return linkedList;
    }
    ...
    public void addKeyUsage(String str) {
        for (int i2 = 0; i2 < str.length(); i2++) {
            if ("01X".indexOf(str.charAt(i2)) == -1) {
    ...
    public List<String> getKeyUsage() {
        ...
        for (COSBase cOSBase : cOSArray) {
            if (cOSBase instanceof COSString) {
                linkedList.add(((COSString) cOSBase).getString());
            }
        }
        return linkedList;
    }
        ...
        for (COSBase cOSBase : list) {
            if (cOSBase instanceof COSDictionary) {
                ...
                for (COSName cOSName : cOSDictionary.keySet()) {
                    map.put(cOSName.getName(), cOSDictionary.getString(cOSName));
                }
                linkedList.add(map);
            }
        }
    ...
    public void addKeyUsage(char c2, char c3, char c4, char c5, char c6, char c7, char c8, char c9, char c10) {
        addKeyUsage("" + c2 + c3 + c4 + c5 + c6 + c7 + c8 + c9 + c10);
    }
}
