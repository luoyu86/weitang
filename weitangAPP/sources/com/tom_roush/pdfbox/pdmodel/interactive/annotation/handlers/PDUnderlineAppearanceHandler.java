...
public class PDUnderlineAppearanceHandler extends PDAbstractAppearanceHandler {
    ...
    public void generateNormalAppearance() {
        ...
        for (int i3 = 0; i3 < quadPoints.length / 2; i3++) {
            int i4 = i3 * 2;
            float f3 = quadPoints[i4];
            float f4 = quadPoints[i4 + 1];
        ...
        try {
            try {
                ...
                while (i2 < quadPoints.length / 8) {
                    ...
                    float f5 = quadPoints[i6];
                    float f6 = quadPoints[i8];
                    if (Float.compare(fSqrt, f2) != 0) {
                        f5 += (((quadPoints[i5] - quadPoints[i6]) / fSqrt) * fSqrt) / 7.0f;
                        f6 += ((quadPoints[i7] - quadPoints[i8]) / fSqrt) * (fSqrt / 7.0f);
                    }
                    int i9 = i5 + 2;
                    int i10 = i5 + 6;
                    int i11 = i5 + 3;
                    ...
                    float f7 = quadPoints[i10];
                    float f8 = quadPoints[i12];
                    if (Float.compare(fSqrt2, 0.0f) != 0) {
                        f7 += (((quadPoints[i9] - quadPoints[i10]) / fSqrt2) * fSqrt2) / 7.0f;
                        f8 += (((quadPoints[i11] - quadPoints[i12]) / fSqrt2) * fSqrt2) / 7.0f;
                    }
                    normalAppearanceAsContentStream.moveTo(f5, f6);
                    normalAppearanceAsContentStream.lineTo(f7, f8);
                    i2++;
                    f2 = 0.0f;
                }
