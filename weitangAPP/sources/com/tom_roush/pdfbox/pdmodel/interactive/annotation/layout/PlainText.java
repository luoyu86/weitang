    ...
    public static class Paragraph {
        ...
        public List<Line> getLines(PDFont pDFont, float f2, float f3) throws IOException {
            ...
            while (true) {
                ...
                String strSubstring = this.textContent.substring(i3, iFirst);
                float stringWidth2 = pDFont.getStringWidth(strSubstring) * f4;
                stringWidth += stringWidth2;
                if (stringWidth >= f3 && Character.isWhitespace(strSubstring.charAt(strSubstring.length() - 1))) {
                    stringWidth -= pDFont.getStringWidth(strSubstring.substring(strSubstring.length() - 1)) * f4;
                }
                if (stringWidth >= f3) {
                    line.setWidth(line.calculateWidth(pDFont, f2));
                    arrayList.add(line);
                    line = new Line();
                    stringWidth = pDFont.getStringWidth(strSubstring) * f4;
                }
