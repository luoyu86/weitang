...
public final class OpenTypeScript {
    ...
    static {
        ...
        UNICODE_SCRIPT_TO_OPENTYPE_TAG_MAP = new HashMap(TTDownloadField.CALL_DOWNLOAD_MODEL_SET_DEEP_LINK);
        ...
        try {
            try {
                ...
                Log.w("PdfBox-Android", "Could not parse Scripts.txt, mirroring char map will be empty: " + e2.getMessage());
    ...
    private static void ensureValidCodePoint(int i2) {
        if (i2 < 0 || i2 > 1114111) {
            throw new IllegalArgumentException("Invalid codepoint: " + i2);
        }
    }

    public static String[] getScriptTags(int i2) {
        ensureValidCodePoint(i2);
        return UNICODE_SCRIPT_TO_OPENTYPE_TAG_MAP.get(getUnicodeScript(i2));
    }

    private static String getUnicodeScript(int i2) {
        ensureValidCodePoint(i2);
        if (Character.getType(i2) == 0) {
            return UNKNOWN;
        }
        int iBinarySearch = Arrays.binarySearch(unicodeRangeStarts, i2);
        if (iBinarySearch < 0) {
            iBinarySearch = (-iBinarySearch) - 2;
        }
        return unicodeRangeScripts[iBinarySearch];
    }
    ...
    private static void parseScriptsFile(InputStream inputStream) throws IOException {
        ...
        while (true) {
            ...
            if (line.length() >= 2) {
                ...
                if (stringTokenizer.countTokens() >= 2) {
                    ...
                    if (iIndexOf2 == -1) {
                        int i2 = Integer.parseInt(strTrim, 16);
                        iArr2[1] = i2;
                        iArr2[0] = i2;
                    } else {
        ...
        for (Map.Entry entry : treeMap.entrySet()) {
            unicodeRangeStarts[i3] = ((int[]) entry.getKey())[0];
            unicodeRangeScripts[i3] = (String) entry.getValue();
            i3++;
        }
    }
}
