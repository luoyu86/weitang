...
public abstract class Folder implements AutoCloseable {
    ...
    private final EventQueue f14924q;
    public Store store;
    public int mode = -1;
    ...
    public Folder(Store store) {
        ...
        if (property.equalsIgnoreCase("application")) {
            this.f14924q = EventQueue.getApplicationEventQueue(executor);
            return;
        }
        if (property.equalsIgnoreCase("session")) {
            this.f14924q = session.getEventQueue();
        } else if (property.equalsIgnoreCase("store")) {
            this.f14924q = store.getEventQueue();
        } else {
            this.f14924q = new EventQueue(executor);
        }
    }

    private void queueEvent(MailEvent mailEvent, Vector<? extends EventListener> vector) {
        this.f14924q.enqueue(mailEvent, (Vector) vector.clone());
    }
    ...
    public synchronized void setFlags(int i2, int i3, Flags flags, boolean z) throws MessagingException {
        while (i2 <= i3) {
            try {
                getMessage(i2).setFlags(flags, z);
            } catch (MessageRemovedException unused) {
            }
            i2++;
        }
