...
public class TypefaceCompatApi26Impl extends TypefaceCompatApi21Impl {
    ...
    public Typeface createFromFontInfo(Context context, @Nullable CancellationSignal cancellationSignal, @NonNull FontsContractCompat.FontInfo[] fontInfoArr, int i2) {
        ...
        if (isFontFamilyPrivateAPIAvailable()) {
            ...
            if (freeze(objNewFamily)) {
                return Typeface.create(createFromFamiliesWithDefault(objNewFamily), i2);
            }
            return null;
        }
        FontsContractCompat.FontInfo fontInfoFindBestInfo = findBestInfo(fontInfoArr, i2);
        try {
            ParcelFileDescriptor parcelFileDescriptorOpenFileDescriptor = context.getContentResolver().openFileDescriptor(fontInfoFindBestInfo.getUri(), PDPageLabelRange.STYLE_ROMAN_LOWER, cancellationSignal);
